 ...
 * RBAC 基础数据初始化脚本（幂等）
 *
 * 功能：创建/更新角色 + 权限 + 角色-权限映射
 * 安全性：可在任何环境安全运行（生产/开发），不包含任何用户数据
 * 使用方式：npm run db:seed:rbac
 */
...
/**
 * 初始化 RBAC 数据（幂等，安全可重复运行）
 * 返回 roleMap 供后续脚本使用
 */
export async function seedRbac(prismaClient?: PrismaClient): Promise<Map<string, string>> {
